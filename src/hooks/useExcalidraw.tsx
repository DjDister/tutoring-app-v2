...
export const useExcalidraw = (
  ...
  const moveIdsStore = useAppSelector(
    ExcalidrawMoveSlice.selectExcalidrawMovedEls
  );
  const [pointerState, setPointerState] = useState<pointerButtonType>(
    pointerStateEnm.UP
  );
  ...
  const handleCurrentItemRoundness = (
    currentItemRoundness: StrokeRoundness
  ) => {
    if (
      currentItemRoundness === StrokeRoundnessEnum.ROUND &&
  ...
  const handle_Remove = (activeEls: ExcalidrawElement[]) => {
    const deletedEls = totalStore.filter(
      ({ id: totalId }: { id: string }) =>
        !activeEls.some(({ id: activeId }) => totalId === activeId)
    );
    const findOwsEls = deletedEls
  ...
  const handle_Reset = () => {
    const findOwsEls = totalStore
      .filter(({ writerId }) => writerId === getUserId)
      .map(({ id }: { id: string }) => id);
    const findOtherEls = totalStore.filter(
      ({ writerId }) => writerId != getUserId
    );
    const isFindOwsEls = Boolean(findOwsEls.length);
    const isfindOtherEls = Boolean(findOtherEls.length);
    ...
    if (ischangeElement && excalidrawRef.current) {
      ...
      if (Boolean(activeElsLeng)) {
        ...
        if (StoreElsLeng < activeElsLeng) {
          const insertFramIdEl = cloneDeep({
            ...activeEls.at(-1),
            writerId: getUserId,
          }) as ExcalidrawSlice.ExcalidrawCustomDTO;
          handleExcalidrawSelectDispatch(ExcalidrawSlice.setAddEl, {
            message: insertFramIdEl,
          });
        ...
        // 02 Move
        if (StoreElsLeng === activeElsLeng) {
          const moveOwnEls = totalStore.filter(({ id }: { id: string }) =>
            moveIdsStore.includes(id)
          );
          const moveOtherEls = totalStore.filter(
            ({ id }: { id: string }) => !moveIdsStore.includes(id)
          );
